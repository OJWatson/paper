...
```{r Animate the infection network, include=TRUE, message = FALSE, warning = FALSE, fig.align = "center", cache=T}
...
paper::offspring_distribution_plot(outbreak.dataset=outbreak.dataset)

```

What we can see from above is the confirmed number of secondary cases from each individual. We have not observed
any zero R0 values because from the nature of the practical it is hard to know exactly whether an individual infected 
zero if they have not ended their infectious period. If we assume that everyone who was infected by yielded no
secondary cases by th enf of the practical we can then observe potential values of R0 equal to 0.

```{r Offspring distribution with zero, include=TRUE, message = FALSE, warning = FALSE, fig.align = "center"}

paper::offspring_distribution_plot(outbreak.dataset=outbreak.dataset, include.unobserved.as.zero = T)
...
To do this we can use the following function to simulate 2000 outbreaks and plot
...
infection and recovery is also drawn from a Poisson with a mean equal to the observed 
mean generation and recovery time respectively. Simulations are then stopped after 
a period of time equal to the length of the observed outbreak.
...
generation and recovery times from the observed outbreak:
...
temporal dynamics exhibited. There are many reasons why this is, but perhaps most
interesting is the clear lull in infections that happens on Wednesdays in the observed
data (when the students have half days for sports matches), which is not captured
by the simulation.
