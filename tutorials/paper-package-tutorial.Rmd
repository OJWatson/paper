...
been collected and stored in a similar fashion to that presented within the xls file
included within the external data directory (the names have been excluded for sensitivity
reasons), and that the main data from the first sheet in the excel document has been
exported to a tab delimited .txt file (also included in the external data directory).
...
First we will read in the data from wherever you have stored the .txt file. Below
we will be using the example included with the package.
...
outbreak.dataset <- paper::outbreak_dataset_read(file = system.file("extdata","paper.txt",package="paper"))
...
# Lastly we can visualise what the outbreak looked like by viewign the outbreak
...
of the above function pleasethen click [here](https://goo.gl/mMDcVo)
